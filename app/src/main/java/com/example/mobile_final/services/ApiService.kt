...
class ApiService private constructor(context: Context) {
    ...
    private val baseUrl: String
        get() = "https://orion-vuz-mobile.vercel.app/"

    private val wsBaseUrl: String
        get() = "wss://orion-vuz-mobile.vercel.app/"
    fun getWebSocketUrl(userId: String): String {
        return "${wsBaseUrl}ws/$userId"
    }
    ...
    suspend fun login(request: LoginRequest): AuthResponse = withContext(Dispatchers.IO) {
        ...
        return@withContext try {
            ...
            if (response.isSuccessful) {
                ...
                val user = if (jsonResponse.has("name")) {
    ...
    suspend fun verify(request: VerifyRequest): AuthResponse=withContext(Dispatchers.IO){
        val json = JSONObject().apply {
